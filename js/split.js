...
const SplitPage = (() => {
            ...
            <tbody>
              ${cards.map(c => {
                ...
                return `<tr style="border-bottom:0.5px solid var(--border)">
                  <td style="padding:3px 10px 3px 0;color:var(--text2)">${c.name}</td>
            ...
            <tbody>
              ${topCats.map(([cat, amt]) => `
                <tr style="border-bottom:0.5px solid var(--border)">
                  <td style="padding:3px 10px 3px 0;color:var(--text2)">${cat}</td>
  ...
  function renderTable() {
    ...
    // 스크롤 위치 복원
    if (wrap) {
      wrap.scrollTop = wasAtBottom ? wrap.scrollHeight : prevScrollTop;
    }
  ...
  function parseDateStr(s) {
    ...
    if (iso) return +iso[2]*100 + +iso[3];
    const slash = s.match(/^(\d{1,2})\/(\d{1,2})/);
    if (slash) return +slash[1]*100 + +slash[2];
    if (/^\d{1,2}$/.test(s)) return 9999; // 일만 입력된 경우
    return 99999;
  }
  ...
  function onKeydown(e) {
    ...
    if (el.classList.contains('tbl-inp') && el.dataset.field === 'date' && e.key === 'Enter') {
      ...
      _rows[+el.dataset.idx].date = normalized;
      scheduleSave();
    }
  }
  ...
  function commitNewRow() {
    ...
    nr.querySelectorAll('.new-inp').forEach(inp => {
      if (inp.dataset.field === 'amount') {
        ...
        row.date = normalizeDate(inp.value) || row.date;
      } else {
  ...
  function memoCardHtml(card, ci) {
    ...
    if (card.type === 'checklist') {
      ...
      body = `<div style="display:flex;flex-wrap:wrap;gap:5px;margin-bottom:6px">
        ${imgs.map((img,ii)=>`
          <div style="position:relative;width:54px;height:40px;border-radius:4px;overflow:hidden;border:0.5px solid var(--border);cursor:zoom-in" onclick="SplitPage.openLightbox('${img.dataUrl}')">
  ...
  function bindMemoCardEvents() {
    ...
    if (!c) return;
    c.querySelectorAll('.mc-title-inp').forEach(inp => {
      inp.oninput = e => { _memo.cards[+e.target.dataset.ci].title = e.target.value; scheduleMemoSave(); };
    });
    c.querySelectorAll('.mc-del').forEach(btn => {
      btn.onclick = () => { _memo.cards.splice(+btn.dataset.ci,1); renderMemoCards(); scheduleMemoSave(); };
    });
    c.querySelectorAll('.mc-add-item').forEach(btn => {
      btn.onclick = () => {
        ...
        renderMemoCards(); scheduleMemoSave();
      };
    });
    c.querySelectorAll('.mc-item-del').forEach(btn => {
      btn.onclick = () => { _memo.cards[+btn.dataset.ci].items.splice(+btn.dataset.ii,1); renderMemoCards(); scheduleMemoSave(); };
    });
    c.querySelectorAll('.mc-inp').forEach(inp => {
      inp.oninput = e => {
    ...
    c.querySelectorAll('.mc-chk').forEach(chk => {
      chk.onchange = e => {
        ...
        if (inp) inp.style.cssText += e.target.checked ? 'text-decoration:line-through;color:var(--text3)' : 'text-decoration:none;color:var(--text1)';
        scheduleMemoSave();
      };
    });
    c.querySelectorAll('.mc-img-input').forEach(inp => {
      inp.onchange = e => {
    ...
    c.querySelectorAll('.mc-img-del').forEach(btn => {
      btn.onclick = e => {
        e.stopPropagation();
        _memo.cards[+btn.dataset.ci].images.splice(+btn.dataset.ii,1);
        renderMemoCards(); scheduleMemoSave();
      };
    });
  }
      ...
      ${withMemo.map((r,i) => `
        <div style="background:var(--bg1);border-radius:5px;padding:6px 8px;margin-bottom:5px;border:0.5px solid var(--border)">
          <div style="display:flex;justify-content:space-between;align-items:flex-start">
            ...
            <button onclick="SplitPage.deleteItemMemo(${r._idx||_rows.indexOf(r)})" style="border:none;background:none;color:var(--text3);cursor:pointer;font-size:12px;padding:0;line-height:1;flex-shrink:0;margin-left:4px">×</button>
          </div>
  ...
  function deleteItemMemo(idx) {
    if (_rows[idx]) { _rows[idx].memo = ''; renderTable(); renderItemMemoSection(); scheduleSave(); }
  }

  function scheduleMemoSave() {
    clearTimeout(_memoSaveTimer);
    _memoSaveTimer = setTimeout(async () => {
      try { await API.saveMemo(APP_STATE.currentMonth, _memo); APP_STATE.memo = JSON.parse(JSON.stringify(_memo)); } catch {}
    }, DEBOUNCE);
  }
  ...
  return { init, openLightbox, deleteItemMemo };
})();